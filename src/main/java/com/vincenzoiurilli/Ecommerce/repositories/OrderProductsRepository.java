...
public interface OrderProductsRepository extends JpaRepository<OrderProducts, UUID> {
    ...
    @Query("SELECT new com.vincenzoiurilli.Ecommerce.dto.products.ProductsStatisticDTO(p.id, p.name, SUM(o.quantity), SUM(o.quantity * o.price_at_purchase)) FROM OrderProducts o JOIN o.product p WHERE p.seller.id = :sellerId GROUP BY p.id, p.name")
